...
int is_straight(struct hand *h)
{
	...
	int max = h->cards[0].val;
	int min = h->cards[0].val;
	for(int i = 0; i < 5; i++) {
		if(h->card_count[h->cards[i].val] = 1) {
			if(h->cards[i].val > max) {
				max = h->cards[i].val;	
			}
			if(h->cards[i].val < min) {
				min = h->cards[i].val;
			}
		}
		else {
...
void eval_strength(struct hand *h)
{
	...
	if(is_straight_flush(h)) {
		//set bit to 55 (StraightFlush)
		h->vector |= BIT(55);
	}
	else if(is_straight(h)) {
		//set bit to 39 (Straight)
		h->vector |= BIT(39);
	}
	else if(is_flush(h)) {
		//set bit to 40 (Flush)
		...
		for(int i = 0; i < sizeof(h->cards); i++) {
			if(h->card_count[h->cards[i].val - 2] = 4) {
				h->vector |= BIT(h->cards[i].val + 40);
			}
			else if(h->card_count[h->cards[i].val - 2] = 3) {
				h->vector |= BIT(h->cards[i].val + 24);
			}
			else if(h->card_count[h->cards[i].val - 2] = 2) {
				for(int j = 26; j < 39; j++) {
					if(IS_SET(h->vector, j)) {
						h->vector |= BIT(41); //FullHouse
					}
				}	
				h->vector |= BIT(h->cards[i].val + 11);
			}
			else if(h->card_count[h->cards[i].val - 2] = 1) {
				h->vector |= BIT(h->cards[i].val - 2);
			}
		}
	}
}
...
void copy_card(struct card *dst, struct card *src)
{
	...
	char temp_val = src->val;
	char temp_suit = src->suit;
	dst->val = temp_val;
	dst->suit = temp_suit;
}
...
void initialize_player_omaha(struct player *p, struct card *player_cards, struct card *community_cards)
{
	...
	for(int i = 0; i < 6; i++) {	
		for(int j = 0; j < 10; j++) {
			if(i = 0) {
				copy_card(&p->hands[j+(10*i)].cards[0], &player_cards[0]);
				copy_card(&p->hands[j+(10*i)].cards[1], &player_cards[1]);
			}
			if(i = 1) {
				copy_card(&p->hands[j+(10*i)].cards[0], &player_cards[0]);
				copy_card(&p->hands[j+(10*i)].cards[1], &player_cards[2]);
			}
			if(i = 2) {
				copy_card(&p->hands[j+(10*i)].cards[0], &player_cards[0]);
				copy_card(&p->hands[j+(10*i)].cards[1], &player_cards[3]);
			}
			if(i = 3) {
				copy_card(&p->hands[j+(10*i)].cards[0], &player_cards[1]);
				copy_card(&p->hands[j+(10*i)].cards[1], &player_cards[2]);
			}
			if(i = 4) {
				copy_card(&p->hands[j+(10*i)].cards[0], &player_cards[1]);
			...
			if(i = 5) {
				copy_card(&p->hands[j+(10*i)].cards[0], &player_cards[2]);
				copy_card(&p->hands[j+(10*i)].cards[1], &player_cards[3]);
			}

			if(j = 0) {
				copy_card(&p->hands[j+(10*i)].cards[2], &community_cards[0]);
				copy_card(&p->hands[j+(10*i)].cards[3], &community_cards[1]);
				copy_card(&p->hands[j+(10*i)].cards[4], &community_cards[2]);
			}
			if(j = 1) {
				copy_card(&p->hands[j+(10*i)].cards[2], &community_cards[0]);
				copy_card(&p->hands[j+(10*i)].cards[3], &community_cards[1]);
				copy_card(&p->hands[j+(10*i)].cards[4], &community_cards[3]);	
			}
			if(j = 2) {
				copy_card(&p->hands[j+(10*i)].cards[2], &community_cards[0]);
				copy_card(&p->hands[j+(10*i)].cards[3], &community_cards[1]);
				copy_card(&p->hands[j+(10*i)].cards[4], &community_cards[4]);
			}
			if(j = 3) {
				copy_card(&p->hands[j+(10*i)].cards[2], &community_cards[0]);
				copy_card(&p->hands[j+(10*i)].cards[3], &community_cards[2]);
				copy_card(&p->hands[j+(10*i)].cards[4], &community_cards[3]);				    }
			if(j = 4) {
				copy_card(&p->hands[j+(10*i)].cards[2], &community_cards[0]);
				copy_card(&p->hands[j+(10*i)].cards[3], &community_cards[2]);
			...
			if(j = 5) {
				copy_card(&p->hands[j+(10*i)].cards[2], &community_cards[0]);
				copy_card(&p->hands[j+(10*i)].cards[3], &community_cards[3]);
				copy_card(&p->hands[j+(10*i)].cards[4], &community_cards[4]);
			}
			if(j = 6) {
				copy_card(&p->hands[j+(10*i)].cards[2], &community_cards[1]);
				copy_card(&p->hands[j+(10*i)].cards[3], &community_cards[2]);
				copy_card(&p->hands[j+(10*i)].cards[4], &community_cards[3]);
			}
			if(j = 7) {
			...
			if(j = 9) {
				copy_card(&p->hands[j+(10*i)].cards[2], &community_cards[2]);
				copy_card(&p->hands[j+(10*i)].cards[3], &community_cards[3]);
				copy_card(&p->hands[j+(10*i)].cards[4], &community_cards[4]);
			}
		}
